        ...
        <>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="text-green-600">{getTypeIcon(material.type)}</div>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getTypeColor(material.type)}`}>{material.type}</span>
                    </div>
                    <span className="text-xs text-gray-500">Sem {material.semester}</span>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-900 font-medium">{material.subject?.name || material.subjectId?.name || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between text-sm">
          ...
          <div className="mt-8 bg-green-50 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{materials.filter(m => m.type === 'notes').length}</div>
                <div className="text-sm text-blue-700">Notes</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-600">{materials.filter(m => m.type === 'assignment').length}</div>
                <div className="text-sm text-purple-700">Assignments</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{materials.filter(m => m.type === 'syllabus').length}</div>
